...
{
    public partial class VehiclesWindow : Form
    {
        /// <summary>
        /// New VehiclesWindow
        /// </summary>
        /// <param name="user"></param>
        public VehiclesWindow(User user)
        {
            this.user = user;
            InitializeComponent();
        }
        ...
        private void RefreshDGV()
        {
            using (SqlConnection Connection = AppConnection.GetConnection())
            {
                Connection.Open();
                Data.Clear();
                SqlCommand Command = new SqlCommand(@"SELECT [id] as [#],[VehicleName] as [وسيلة النقل]
                FROM [" + Settings1.Default.DatabaseName + "].[dbo].[Vehicles]", Connection);
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                using (SqlConnection Connection = AppConnection.GetConnection())
                {
                    ...
                    if (NewCar)
                    {
                        SqlCommand Command = new SqlCommand(@"INSERT INTO [" + Settings1.Default.DatabaseName + @"].[dbo].[Vehicles]
                        ([VehicleName]) VALUES (@VehicleName)", Connection);
                        Command.Parameters.Add("@VehicleName", SqlDbType.NVarChar).Value =  ReplaceChar.ReplaceAChar(textBox2.Text);
                    ...
                    {
                        SqlCommand Command = new SqlCommand(@"UPDATE [" + Settings1.Default.DatabaseName + @"].[dbo].[Vehicles]
                        SET [VehicleName]=@VehicleName WHERE [id]=@id", Connection);
                        Command.Parameters.Add("@VehicleName", SqlDbType.NVarChar).Value =  ReplaceChar.ReplaceAChar(textBox2.Text);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (DR == DialogResult.Yes)
            {
                ...
                {
                    Connection.Open();
                    SqlCommand Command = new SqlCommand(@"DELETE FROM [" + Settings1.Default.DatabaseName + @"].[dbo].[Vehicles]
